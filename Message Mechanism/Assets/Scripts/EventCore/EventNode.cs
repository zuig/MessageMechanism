...
public class EventNode : MonoBehaviour
{
    ...
    /// 卸载一个事件监听器
    /// </summary>
    /// <param name="key">监听器ID</param>
    /// <param name="listener">监听器</param>
    /// <returns></returns>
    public bool DetchEventListener(int key, IEventListener listener)
    {
    ...
    {
        if (!gameObject.activeSelf || !gameObject.activeInHierarchy || !enabled)
            return false;
        if (!mListeners.ContainsKey(key))
            return false;
